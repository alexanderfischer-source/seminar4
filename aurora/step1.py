...
def write_csv(self, rows: List[Dict[str, Any]], output_path):
    if not rows:
        print('Warnin: No rows on output.')
        return
...
class AuroraApp:
    def __init__(self, args: argparse.Namespace):
        ...
        self.processor = DataProcessor(args.repo)



    def run(self):
        print("Loadind and fiktering data...")
...
def parse_args():
    ...
    p.add_argument('--input_file', required=True, help='Path to input CSV file')
    p.add_argument('--output_file', required=True, help='Path to output CSV file')
    ...
    p.add_argument('--triage', help = "Comma separated list of triage priorities, e.g.")
    ...
    p.add_argument('--max-money', type=int, help = "Maximum money-moved-usd")
    ...
    p.add_argument('--sort-desc', action='store_true', help = "Sort by descending order")
